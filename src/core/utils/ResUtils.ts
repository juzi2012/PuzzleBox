module core {
    ...
    export class ResUtils {
        ...
        private static parsedSkinClasses:any = {};

        public static loadExmls(exmlPathsList:Array<Array<string>>, callBack?:()=>void, thisObj?:any):void
        {
            callBack.apply(thisObj);
            // let needLoadExmls = new Array<string>();
            // for(var i:number = 0; i < exmlPathsList.length; i++){
            //     for(var j:number = 0; j < exmlPathsList[i].length; j++){
            //         let skinUrl:string = exmlPathsList[i][j];
            //         if(skinUrl in ResUtils.parsedSkinClasses == false){
            //             needLoadExmls.push(skinUrl);
            //             ResUtils.parsedSkinClasses[skinUrl] = true;
            //         }
            //     }
            // }
            // if(needLoadExmls.length == 0){
            //     if(callBack){
            //         callBack.apply(thisObj);
            //     }
            // }else{
            //     EXML.$loadAll(needLoadExmls, callBack, thisObj)
            // }
        }
    }
